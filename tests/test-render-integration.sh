...
WS_MIGRATION="../workspace/sentinel/migrations/004_hunt_session.sql"
...
trap "docker rm -f $CID >/dev/null; rm -rf /tmp/hunt-out" EXIT
...
INSERT INTO hunt_session (session_id, started_at, ended_at, party_name, encounter, party,
  drops, moves, xp_awarded, rarity, status)
VALUES
...
mkdir -p /tmp/hunt-out
OUT_DIR=/tmp/hunt-out bash scripts/render.sh

[[ -f /tmp/hunt-out/k/sp-won.html ]]   || { echo "missing sp-won"; exit 1; }
[[ -f /tmp/hunt-out/k/sp-grey.html ]]  || { echo "missing sp-grey"; exit 1; }
[[ -f /tmp/hunt-out/k/latest.html ]]   || { echo "missing latest"; exit 1; }

# latest must redirect to the non-grey row
grep -q 'url=sp-won.html' /tmp/hunt-out/k/latest.html || { echo "latest wrong target"; exit 1; }
