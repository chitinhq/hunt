...
[[ -f "$WS_MIGRATION" ]] || { echo "SKIP: $WS_MIGRATION not found (run from quest repo with workspace sibling)"; exit 0; }
...
# Soulforge↔Quest boundary 2a: soul names MUST NOT leak into public HTML party strip.
# Fixture seeds party with soul="sun-tzu"; rendered HTML must show class "Time Mage" instead.
if grep -q 'sun-tzu' /tmp/quest-out/q/sp-won.html; then
  echo "BOUNDARY VIOLATION: soul name 'sun-tzu' rendered in public HTML"; exit 1
fi
grep -q 'Time Mage' /tmp/quest-out/q/sp-won.html || { echo "class 'Time Mage' missing from party strip"; exit 1; }
